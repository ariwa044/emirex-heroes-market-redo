...
function PlansPage() {
  ...
  async function invest(event: React.FormEvent) {
    ...
    const { error } = await supabase.from("investments").insert({ user_id: user.id, plan_id: selected.id, amount: value, expected_return: Number((value * (1 + Number(selected.roi_percent) / 100)).toFixed(2)), ends_at: ends });
    setBusy(false);
    if (error) { toast.error(error.message); return; }
    toast.success(`Invested $${value.toFixed(2)} in the ${selected.name} plan.`);
  ...
  return <AccountShell eyebrow="Grow" title="Investment plans">
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      {plans.map((plan) => <article key={plan.id} className={`rounded-2xl border bg-card p-6 transition-colors ${selected?.id === plan.id ? "border-signal" : "border-border"}`}>
        ...
        <Button className="mt-5 w-full" variant={selected?.id === plan.id ? "default" : "outline"} onClick={() => { setSelected(plan); setAmount(String(plan.min_amount)); }}>Select plan</Button>
      </article>)}
    </div>
    ...
    {selected && <form onSubmit={invest} className="mt-3 rounded-2xl border border-signal/40 bg-signal-soft p-6">
      ...
      <h2 className="mt-4 font-head text-xl font-semibold">Invest in {selected.name}</h2>
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end">
        ...
        <Button type="submit" disabled={busy}>{busy ? "Processing…" : "Confirm investment"}</Button>
        <Button type="button" variant="ghost" onClick={() => setSelected(null)}>Cancel</Button>
      </div>
      <p className="mt-3 text-xs text-muted-foreground">Projected payout ${(Number(amount || 0) * (1 + Number(selected.roi_percent) / 100)).toFixed(2)} after {selected.duration_days} days.</p>
    </form>}

    <section className="mt-3 rounded-2xl border border-border bg-card p-6">
      <h2 className="font-head text-xl font-semibold">Your investments</h2>
      {investments.length === 0 ? <p className="mt-4 text-sm text-muted-foreground">You have no active investments yet.</p> : <ul className="mt-4 divide-y divide-border">
        {investments.map((row) => <li key={row.id} className="flex flex-wrap items-center justify-between gap-2 py-3 text-sm">
          <div><p className="font-medium">{planName(row.plan_id)} · ${Number(row.amount).toFixed(2)}</p><p className="text-xs text-muted-foreground">Started {new Date(row.started_at).toLocaleDateString()}{row.ends_at ? ` · Matures ${new Date(row.ends_at).toLocaleDateString()}` : ""}</p></div>
          <div className="text-right"><p className="font-medium text-positive">${Number(row.expected_return).toFixed(2)}</p><p className="text-[11px] uppercase tracking-wide text-muted-foreground">{row.status}</p></div>
        </li>)}
      </ul>}
    </section>
  </AccountShell>;
