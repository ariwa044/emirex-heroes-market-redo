...
type Profile = { user_id: string; display_name: string; username: string | null; upgrade_required: boolean; account_on_hold: boolean; upgrade_notice: string | null; manual_profit: number | null; manual_withdrawals: number | null; manual_bitcoin: number | null; manual_balance: number | null; manual_invested: number | null };
...
function AdminPage() {
  ...
  const [tileDraft, setTileDraft] = useState({ balance: "", invested: "", profit: "", withdrawals: "", bitcoin: "" });
  ...
  const load = useCallback(async () => {
    const [{ data: s }, { data: p }, { data: t }, { data: i }, { data: pl }] = await Promise.all([
      ...
      supabase.from("profiles").select("user_id,display_name,username,upgrade_required,account_on_hold,upgrade_notice,manual_profit,manual_withdrawals,manual_bitcoin,manual_balance,manual_invested"),
  ...
  async function saveTiles(p: Profile) {
    ...
    const { error } = await supabase.from("profiles").update({
      ...
      manual_balance: parse(tileDraft.balance, p.manual_balance),
      manual_invested: parse(tileDraft.invested, p.manual_invested),
    }).eq("user_id", p.user_id);
    setBusy(false);
    if (error) { toast.error(error.message); return; }
    toast.success("Member dashboard figures updated.");
    setTileDraft({ balance: "", invested: "", profit: "", withdrawals: "", bitcoin: "" });
    void load();
  }

  async function clearTiles(p: Profile) {
    setBusy(true);
    const { error } = await supabase.from("profiles").update({ manual_profit: null, manual_withdrawals: null, manual_bitcoin: null, manual_balance: null, manual_invested: null }).eq("user_id", p.user_id);
    setBusy(false);
    if (error) { toast.error(error.message); return; }
